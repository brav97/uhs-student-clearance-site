...
if(!isset($_SESSION['maths_dept_cleared']))
{ 
      ...
      <div class="container-fluid">
        ...
        <div class="row jumbotron">
          <div class="col-lg-8 col-md-12 col-xs-12 col-sm-12" style="background-color: white;border:1px solid black;padding: 40px 20px;">
            <h4><b>LANGUAGES DEPARTMENT CHECKLIST</b></h4><hr><br>
            <?php
              include '../db/db-connection.php';
              global $adm;

              $select = "SELECT * FROM `borrowed_items` WHERE `adm_number` = '$adm' AND `status` = 'BORROWED' AND `department` = 'languages'";
 ...
 <?php     
            ...
            if(isset($_POST['check-maths-dept'])){
              ...
              if($rows >= 1){
                ...
                $num = 1;
                while ($data = mysqli_fetch_assoc($query)) { 
                  $status = $data['languages']; 
                  if($status == "WAITING"){
                    echo "<script>alert('THIS IS THE SECOND STEP OF YOUR CLEARANCE. YOU ARE REQUIRED TO SUBMIT ALL DETAILS LISTED BELOW BEFORE PROGRESSING ');</script>";
                    ...
                    echo $status;
                    $_SESSION['languages_dept_cleared'] = 'PROCEEDTOSCIENCES';
