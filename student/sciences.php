...
if(!isset($_SESSION['languages_dept_cleared']))
{ 
  echo "<script>window.location.replace('languages.php');</script>";
}else{
      ...
      <div class="container-fluid">
        ...
        <div class="row jumbotron">
          <div class="col-lg-8 col-md-12 col-xs-12 col-sm-12" style="background-color: white;border:1px solid black;padding: 40px 20px;">
            <h4><b>SCIENCES DEPARTMENT CHECKLIST</b></h4><hr><br>
            <?php
              include '../db/db-connection.php';
              global $adm;

              $select = "SELECT * FROM `borrowed_items` WHERE `adm_number` = '$adm' AND `status` = 'BORROWED' AND `department` = 'sciences'";
 ...
 <?php     
            ...
            if(isset($_POST['check-maths-dept'])){
              ...
              if($rows >= 1){
                ...
                $num = 1;
                while ($data = mysqli_fetch_assoc($query)) { 
                  $status = $data['sciences']; 
                  if($status == "WAITING"){
                    echo "<script>alert('THIS IS THE THIRD STEP OF YOUR CLEARANCE. YOU ARE REQUIRED TO SUBMIT ALL DETAILS LISTED BELOW BEFORE PROGRESSING ');</script>";
                    ...
                    echo $status;
                    $_SESSION['sciences_dept_cleared'] = 'PROCEEDTOHUMANITIES';
                    echo "<script>window.location.replace('humanities.php');</script>";
                  }
