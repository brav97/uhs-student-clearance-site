...
<body id="page-top">
      ...
      <div id="content">
        ...
        <nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow">
          ...
          <form class="d-none d-sm-inline-block form-inline mr-auto ml-md-3 my-2 my-md-0 mw-100 navbar-search" method="post" action="" >
            <div class="input-group">
              <input type="number" class="form-control bg-light border-0 small" placeholder="Search for..." name="registration" >
              <div class="input-group-append" >
                <button class="btn btn-primary" type="submit" name="search-student">
        ...
        <div class="container-fluid">
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">Cleared Students</h1>
            ...
            <div class="col-lg-12 col-md-12 col-xs-12 col-sm-12">
              ...
              global $adm;
                if(isset($_POST['search-student'])){
                  ...
                  $student = mysqli_real_escape_string($conn, $_POST['registration']);
                  if(empty($student)){
                    echo "<script>alert('PROVIDE THE STUDENT REGISTRATION NUMBER');</script>";
                  }else{
                     $select = "SELECT * FROM `borrowed_items` WHERE `department` = '$select_category' AND `adm_number` = '$student'";
              ...
              if($rows >= 1){
                ...
                echo "<center><h4><i>THIS STUDENT DOES NOT HAVE PENDING CLEARANCE ITEMS</i></h4></center>";
              }
                  }
                }

              $select = "SELECT * FROM `clearance_status` WHERE `$select_category` = 'WAITING'";
              $query = mysqli_query($conn, $select);
              $rows = mysqli_num_rows($query);
              if($rows >= 1){
                echo"
                ...
                while ($data = mysqli_fetch_assoc($query)) {
                  ...
                  echo "
                    ...
                    <tr>
                      ...
                      <td>$adm</td>
                      <td>$fullnames</td>
                      <td><a href='all-students.php?reg=$adm' class='btn btn-block btn-success'>Verify </a></td>
                    </tr>
                  ";
                  $num++;
                }
                ...
                echo "<center><h4><i>NO ANY PENDING  CLEARANCE IN THIS DEPARTMENT</i></h4></center>";
              }
            ?>
                </div>
            </div>
 ...
 <?php            if(isset($_GET['reg'])){
                  $id = $_GET['reg'];
                  include '../db/db-connection.php'; 
                    $select = "UPDATE `clearance_status` SET `$select_category` = 'CLEARED' WHERE `adm` = '$id'";
                  $query = mysqli_query($conn, $select);
                  if($query){
